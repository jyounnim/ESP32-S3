...
FreeRTOS와 마찬가지로 Zephyr의 `k_sem`도 초기값과 최댓값을 다르게 주면 그대로 Counting Semaphore가 됩니다. 별도의 다른 자료형이 필요 없습니다 — Binary Semaphore(count 1)와 Counting Semaphore는 `K_SEM_DEFINE`에 넘기는 숫자만 다를 뿐, 완전히 같은 API를 씁니다.
