...
05번 실습의 우선순위 역전 문제를, `k_sem` 대신 `k_mutex`로 바꿔서 완화해봅니다. Zephyr의 `k_mutex`는 **기본적으로 Priority Inheritance가 내장**되어 있습니다 — FreeRTOS처럼 "Binary Semaphore vs Mutex 중 뭘 쓸지 선택"하는 게 아니라,애초에 자료형 자체가 다르고 Mutex 쪽에 상속 로직이 항상 포함되어 있습니다.
