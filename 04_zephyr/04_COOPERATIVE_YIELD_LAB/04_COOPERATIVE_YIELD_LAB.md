...
02번 실습에서 "협조적 스레드는 스스로 양보하기 전까지 절대 선점되지 않는다"는 걸 확인했습니다. 이번엔 그게 **같은 우선순위의 협조적 스레드끼리도 마찬가지**라는 걸 확인합니다. FreeRTOS는 같은 우선순위 Task끼리 스케줄러가 자동으로 time-slice(교대)해주지만, **Zephyr의 협조적 스레드는 이 자동 교대 대상에서 아예 제외**됩니다.
...
- FreeRTOS 커리큘럼의 04번 실습(Task Watchdog)과 이 실습을 비교해보세요 — FreeRTOS는 "yield 안 하면 워치독이 강제로 재부팅시킨다"는 안전장치가 있었지만, Zephyr의 협조적 스레드 굶주림은 **기본적으로 조용히 멈출 뿐, 자동으로 감지·복구되지 않습니다**. 그만큼 협조적 스레드를 쓸 때는 개발자가 스스로 yield 지점을 잘 설계해야 할 책임이 더 큽니다
