...
FreeRTOS 커리큘럼과 동일한 문제를 Zephyr에서 재현합니다. 여기서는 세 스레드 모두 **선점형(우선순위 0 이상)**으로 만들어서, 04번에서 다룬 협조적 스레드의 특수성과 이 문제(우선순위 역전)를 분리해서 볼 수 있게 했습니다.
...
- `k_sem`은 FreeRTOS의 Binary Semaphore와 마찬가지로 "소유자" 개념이 없는 단순 신호 장치라, 스케줄러가 `ThreadL`을 배려해서 승격시켜줄 방법이 없습니다
