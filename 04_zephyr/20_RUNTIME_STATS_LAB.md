...
06번 실습에서는 우선순위 낮은 스레드로 CPU 유휴 시간을 수작업으로 흉내 냈습니다. Zephyr는 이를 위한 **공식 API**를 제공합니다. FreeRTOS 커리큘럼 20번(ESP-IDF의 코어별 Idle Hook)과 목적은 비슷하지만, Zephyr는 ESP-IDF 전용 API가 아니라 **모든 보드에서 동일하게 쓸 수 있는 이식성 있는 표준 API**로 이를 제공한다는 차이가 있습니다.
