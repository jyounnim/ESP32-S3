...
FreeRTOS의 Queue에 해당하는 Zephyr의 기본 자료구조는 `k_msgq`입니다. 고정 크기 항목을 FIFO로 주고받는다는 점에서 개념은 동일합니다.
...
- `k_msgq_put`의 대기시간을 `K_MSEC(100)`으로 줬습니다 — `ProcessThread`를 없앤 채로 실행하면, 5개가 다 차고 나서 "queue full, send failed"가 나오는지 확인해보세요 (10번 실습 FreeRTOS 버전과 동일한 실험)
