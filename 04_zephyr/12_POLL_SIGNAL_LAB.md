...
FreeRTOS의 Task Notification(값 하나를 가볍게 전달하는 내장 슬롯)과 가장 비슷한 Zephyr 기능이 **Poll Signal**입니다. 별도의 Queue/Semaphore 객체 없이, `k_poll`과 짝을 이루어 "값이 있는 가벼운 1회성 이벤트"를 전달합니다.
...
- Poll Signal 하나만 놓고 보면 Semaphore와 큰 차이가 없어 보일 수 있지만, 진짜 가치는 **11번 실습의 `k_poll`과 결합했을 때** 드러납니다 — Message Queue, Semaphore, Poll Signal을 전부 같은 `k_poll` 배열에 섞어 넣고 "이 중 아무거나 오면 처리"하는 통합된 대기 지점을 만들 수 있습니다. FreeRTOS의 Task Notification은 이런 식으로 다른 IPC 객체와 한 번에 묶어서 기다릴 수 있는 방법이 없습니다
