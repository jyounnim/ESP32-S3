...
FreeRTOS 16번 실습과 동일한 문제를 Zephyr의 `k_mutex`로 재현합니다. Mutex 두 개를 서로 다른 순서로 잠그면 순환 대기(Deadlock)에 빠진다는 원리 자체는 RTOS 종류와 무관한 보편적인 문제입니다.
...
- 이 문제와 해결책은 FreeRTOS 16번 실습과 완전히 동일한 원리입니다 — **Lock Ordering(잠금 순서 통일)**은 RTOS 종류와 무관하게 통하는 범용적인 회피 전략이라는 걸 보여줍니다
