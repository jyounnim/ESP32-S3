...
FreeRTOS 07번 실습과 목표는 같지만(ISR에서 신호만 주고, 실제 처리는 Thread에서), Zephyr의 GPIO 다루는 방식 자체가 다릅니다. Zephyr는 핀 번호를 코드에 직접 쓰지 않고 **Devicetree**로 하드웨어를 선언합니다. 그리고 결정적으로, **`k_sem_give()`는 ISR 안에서 별도 전용 함수 없이 그냥 그대로 호출**할 수 있습니다 — FreeRTOS가 `xSemaphoreGiveFromISR` + `portYIELD_FROM_ISR`을 따로 요구했던 것과 다릅니다.
...
| `k_sem_give(&sem)` | **ISR 안에서 그대로 호출 가능** — FreeRTOS처럼 별도의 `FromISR` 버전이 필요 없음 |
...
- FreeRTOS는 ISR용 API(`xSemaphoreGiveFromISR`)와 일반 Task용 API(`xSemaphoreGive`)가 분리되어 있고, `portYIELD_FROM_ISR`까지 직접 챙겨야 했습니다. Zephyr는 `k_sem_give()` **하나로 두 컨텍스트 모두 처리**됩니다 — "깨어난 스레드가 즉시 실행되어야 하는가"는 커널이 알아서 판단합니다. API가 통일되어 있다는 게 이번 실습의 핵심 포인트입니다
- 반면 09번 실습에서 다룰 `k_mutex`는 **ISR에서 절대 사용할 수 없습니다** (Lock/Unlock 모두 금지) — Mutex는 "소유자"라는 개념이 있는데 ISR에는 그 개념이 자연스럽게 적용되지 않기 때문입니다. "이벤트 신호는 Semaphore, 자원 보호는 Mutex"라는 원칙은 Zephyr에서도 FreeRTOS와 동일합니다
