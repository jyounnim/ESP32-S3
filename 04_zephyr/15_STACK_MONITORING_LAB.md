...
FreeRTOS의 `uxTaskGetStackHighWaterMark`에 대응하는 Zephyr API는 `k_thread_stack_space_get`입니다. 단위가 **word가 아니라 바이트**라는 점만 다르고 개념은 동일합니다.
...
- `HeavyThread`의 `STACK_SIZE`를 `2048`에서 `512`로 줄여보세요 — 여유분이 0에 가까워지거나, Zephyr의 내장 스택 보호 기능(하드웨어 MPU 가드 또는 canary, 보드/설정에 따라 다름)에 의해 `Stack overflow` 계열의 fatal error가 발생할 수 있습니다. 이 보호 기능은 FreeRTOS의 `Stack canary watchpoint triggered`와 목적이 같습니다 (확인 후 다시 2048로 복구하세요)
- FreeRTOS는 스택 크기를 **word(4바이트) 단위**로 지정했지만, Zephyr는 **바이트 단위**로 지정한다는 점을 항상 기억하세요 — 코드를 포팅할 때 이 부분에서 실수하기 쉽습니다
