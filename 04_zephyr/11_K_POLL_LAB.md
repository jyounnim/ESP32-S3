...
FreeRTOS 11번 실습(Queue Set)과 같은 목적("여러 이벤트 소스 중 아무거나 먼저 오는 걸 처리")을 Zephyr는 **`k_poll`**이라는 훨씬 더 범용적인 API로 해결합니다. Queue Set이 "큐/세마포어"로 대상이 제한적이었다면, `k_poll`은 Message Queue, Semaphore, Poll Signal(12번) 등 **다양한 종류의 커널 객체를 한 번에, 섞어서** 기다릴 수 있습니다.
...
- `k_poll`은 `K_POLL_TYPE_SEM_AVAILABLE`(세마포어), `K_POLL_TYPE_SIGNAL`(12번에서 다룰 Poll Signal) 등 **서로 다른 종류의 이벤트를 같은 배열에 섞어서** 기다릴 수 있습니다 — FreeRTOS의 Queue Set이 "큐/세마포어류"로 제한적이었던 것보다 더 넓은 범용성을 갖습니다
