...
FreeRTOS 커리큘럼 19번(`19_LIGHT_SLEEP_LAB.md`)에서 "Arduino/PlatformIO는 미리 컴파일된 라이브러리를 쓰기 때문에 `sdkconfig`의 Tickless Idle 옵션을 사용자가 직접 켤 수 없어서, `esp_light_sleep_start()`를 코드에서 직접 호출해야 했다"고 배웠습니다. **Zephyr는 처음부터 소스에서 직접 빌드하기 때문에 이 제약이 없습니다** — `prj.conf`에 옵션 하나만 켜면, 코드를 전혀 바꾸지 않고도 시스템이 자동으로 절전 모드에 들어갑니다.
...
- 애플리케이션 코드는 평소처럼 `k_sleep()`을 쓰면 됩니다 — **절전 모드 진입/해제를 위한 별도 API 호출이 필요 없습니다**. 이게 FreeRTOS/Arduino 조합과의 결정적 차이입니다
...
- FreeRTOS/Arduino 조합에서는 "언제, 얼마나 절전 모드에 들어갈지"를 개발자가 `esp_light_sleep_start()` 호출 시점으로 직접 결정해야 했습니다. Zephyr는 **커널이 알아서 판단**합니다 — 다음 깨어날 시점(다음 `k_sleep` 만료, 다음 타이머 만료 등)을 스케줄러가 이미 알고 있기 때문에 가능한 설계입니다
