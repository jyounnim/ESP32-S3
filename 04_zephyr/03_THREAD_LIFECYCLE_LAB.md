...
`K_THREAD_DEFINE`은 스레드 개수가 고정적일 때 편하지만, 실행 중에 필요할 때마다 만들고 없애야 하는 상황(작업 큐 워커 등)에는 `k_thread_create`를 씁니다. FreeRTOS의 `xTaskCreate`/`vTaskDelete`와 개념은 같지만, Zephyr는 **스레드가 쓸 스택 메모리를 미리 별도로 선언**해야 한다는 차이가 있습니다.
...
| 스레드 함수가 `return`하는 것 | 그 자체로 스레드 종료 (FreeRTOS의 `vTaskDelete(NULL)`과 동일한 효과) |
| `k_thread_abort(tid)` | 다른 스레드를 외부에서 강제 종료 (FreeRTOS의 `vTaskDelete(handle)`과 동일) |
...
- FreeRTOS의 `xTaskCreate`는 스택까지 내부적으로 힙에서 알아서 할당해주지만, Zephyr는 `K_THREAD_STACK_DEFINE`으로 **스택 공간을 개발자가 직접 준비**해야 합니다 — 이는 Zephyr가 메모리 사용을 더 명시적으로/예측 가능하게 만들려는 설계 철학과 관련이 있습니다 (특히 메모리가 극히 제한된 소형 MCU를 함께 지원하기 위함)
